...
SELECT
    ...
    (
        ...
        WHERE date_trunc('month', e.date) = date_trunc('month', CURRENT_DATE - INTERVAL '1 month')
        GROUP BY c.name
        ...
        ELSE (
            (
                ...
                AND c.name = (
                    ...
                    WHERE date_trunc('month', e2.date) = date_trunc('month', CURRENT_DATE - INTERVAL '1 month')
                    GROUP BY c2.name
...
COMMENT ON VIEW api.expense_summary IS 'Provides summary data for the reports page';
COMMENT ON VIEW api.category_comparison IS 'Provides category distribution data for all time';
